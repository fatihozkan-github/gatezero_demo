...
class UILists {
  ...
  static List<String> instruction_images = [
    UIAssets.qrPagePlasticImage,
    UIAssets.qrPageDryImage,
    UIAssets.qrPageSmallImage,
    UIAssets.qrPageBagImage,
  ];
  ...
  static List<DropdownMenuItem> feedbackAppDropdown = [
    ...
    DropdownMenuItem(
        value: 'Herostation haritada gözükmüyor / HeroStation bilgileri doğru gözükmüyor',
        child: Text('Herostation haritada gözükmüyor / HeroStation bilgileri doğru gözükmüyor')),
    DropdownMenuItem(
  ...
  static List<Widget> instructionItems = qrPageInstructions
      .map((item) => Container(
            padding: EdgeInsets.all(8.0),
            decoration: BoxDecoration(borderRadius: BorderRadius.all(Radius.circular(20.0)), color: UIColors.primaryColor),
            child: Center(
              child: Row(
                children: [
                  if (instruction_images[qrPageInstructions.indexOf(item)] != '')
                    Container(
                      margin: EdgeInsets.all(8),
                      child: Image.asset(instruction_images[qrPageInstructions.indexOf(item)], width: 60, fit: BoxFit.contain),
                    ),
                  Expanded(
                    child: Text(
                      item,
